...
def facet_plot_trace_and_templates(
    ...
    for ir, ax in zip(range(df.shape[0]), axs.flatten()):
        ...
        stim = (stim_map[df.iloc[ir, :]['stim']]
                if 'stim' in df.columns else 'OFF')
        tcolor = '#ff5555' if stim == 'ON' else '#3333ff'

        ax.set_title(f"{block=}, {trial=}, stim={stim}",
                     color=tcolor)
...
if __name__ == '__main__':
    ...
    for pth in folders:  # files:
        ...
        fig = facet_plot_trace_and_templates(df, trace_col='trace_let',
                                             template_col='template_pix')
