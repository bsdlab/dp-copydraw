...
def get_nextblock_metadata(save_dir=None, paradigm_name='copyDraw'):
    ...
    if save_dir is None:
        logger.info('no save directory passed to the function looking for existing save files - get_nextblock_metadata()')
    ...
    if not os.path.exists(save_dir):
        ...
        all_files = [file for file in save_dir.iterdir()
                        if file.match('*%s*' % block_name)]
