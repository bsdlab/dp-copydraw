...
class CopyDraw:
    def __init__(
        ...
        raise_on_missing = False if serial_nr is None else True
        self.vpp = VPPort(serial_nr, raise_on_missing_serial=raise_on_missing)
        ...
        self.paths = {
            ...
            "instructions": sp.joinpath(
                "copydraw/assets/instructions/instructions.png"
            ),
            "data": Path(data_dir),
            "script_root": sp,
    ...
    def init_screen(self, win_color: tuple[int, int, int] = (-1, -1, -1)):
        self.win = visual.Window(
            ...
            fullscr=False,
            size=(
                self.win_settings["screen_size"]
                if self.win_settings["screen_size"] is not None
                else (800, 600)
            ),
            units="norm",
    ...
    def set_block_settings(
        ...
        size=0.5,  # scaling factor applied to the template -> by default smaller than full screen!
        interp=True,
        shuffle=True,
        ...
        if block_nr is None:
            block_idx, block_name = get_nextblock_metadata(self.paths["results_path"])
    ...
    def init_block(self):
        ...
        self.trials_vec = {"lengths": [], "id": []}  # n_letters  # index

        # trial index
        self.trial_idx = 1

        # create template order
        self.stimuli["order"] = create_template_order(self.stimuli, self.block_settings)
    ...
    def exec_block(
        ...
        int
    ):  # TODO: Make this return a subprocess which is running the whole psychcopy stuff
    ...
    def scale_stimuli(self):
        ...
        self.stimuli["the_box"], _ = scale_to_norm_units(
            self.stimuli["the_box"], scaling_matrix=self.stimuli["scaling_matrix"]
        )
        ...
        if self.stimuli["flip"]:
            ...
            self.stimuli["current_stim"] = np.matmul(
                self.stimuli["current_stim"], np.array([[1, 0], [0, -1]])
            )
            self.stimuli["the_box"] = np.matmul(
                self.stimuli["the_box"], np.array([[1, 0], [0, -1]])
            )
    ...
    def get_stimuli(self, stimuli_idx: int, scale=True):
        ...
        if self.block_settings["interp"]:
            ...
            self.stimuli["current_stim"] = smooth(
                self.stimuli["current_stim"], return_df=False
            )
    ...
    def create_frame(self, stimuli_idx, scale=True, for_calibration: bool = False):
        ...
        self.frame["elements"]["the_box"] = create_element(
            "the_box", win=self.win, vertices=self.stimuli["the_box"]
        )
        ...
        self.frame["trace_vertices"][0] = convertToPix(
            self.trial_settings["cursor_start_pos"]
            * template_scaling
            * self.block_settings["size"],
            (0, 0),
        ...
        self.frame["elements"]["instructions"] = create_element(
            "instructions", win=self.win, image=self.paths["instructions"]
        )
        ...
        self.frame["elements"]["start_point"] = create_element(
            ...
            size=(
                start_point_size,
                start_point_size * self.win_settings["aspect_ratio"],
            ),
        )

        self.frame["elements"]["time_bar"] = create_element("time_bar", win=self.win)

    def add_calibration_frame_elements(
        self, stimuli_idx: int, scale: bool, size: float = 1.5
    ):
        vert = self.get_stimuli(stimuli_idx, scale=scale)
        self.frame["elements"]["old_template"] = visual.shape.ShapeStim(
            win=self.win,
            vertices=vert,
            lineWidth=100,
            closeShape=False,
        ...
        xx, yy = np.meshgrid(
            np.linspace(vert[:, 0].min() * scale, vert[:, 0].max() * scale, 10),
            np.linspace(vert[:, 1].min() * scale, vert[:, 1].max() * scale, 10),
        )
        for i, (x, y) in enumerate(zip(xx.flatten(), yy.flatten())):
            self.frame["elements"][f"psych_grid_{i}"] = visual.circle.Circle(
                self.win,
                radius=(0.01 / 2, 0.017 / 2),
                pos=(x, y),
                fillColor=None,
                lineColor="red",
            )
        ...
        self.frame["elements"]["topleft"] = visual.circle.Circle(
            self.win,
            radius=(0.02 / 2, 0.034 / 2),
            pos=(vert[:, 0].min() * scale, vert[:, 1].max() * scale),
            fillColor=None,
            lineColor="red",
        )
        self.frame["elements"]["topright"] = visual.circle.Circle(
            self.win,
            radius=(0.02 / 2, 0.034 / 2),
            pos=(vert[:, 0].max() * scale, vert[:, 1].max() * scale),
            fillColor=None,
            lineColor="red",
        )
        self.frame["elements"]["botleft"] = visual.circle.Circle(
            self.win,
            radius=(0.02 / 2, 0.034 / 2),
            pos=(vert[:, 0].min() * scale, vert[:, 1].min() * scale),
            fillColor=None,
            lineColor="red",
        )
        self.frame["elements"]["botright"] = visual.circle.Circle(
            self.win,
            radius=(0.02 / 2, 0.034 / 2),
            pos=(vert[:, 0].max() * scale, vert[:, 1].min() * scale),
            fillColor=None,
            lineColor="red",
        )
    ...
    def exec_trial(self, stimuli_idx, scale=True):
        ...
        self.trial_settings["trial_duration"] = (
            self.block_settings["n_letters"] * self.block_settings["letter_time"]
        )

        # initialise the frame
        self.create_frame(
            self.stimuli["order"][stimuli_idx],
            scale=scale,
        ...
        ptt, start_t_stamp, cursor_t = self._run_trial_main_loop(
            clock, time_bar_x, stimuli_idx
        )
        ...
        trace_let = np.concatenate(
            [
                self.frame["elements"][tr_n].vertices.copy()
                for tr_n in self.frame["traces"]
            ]
        )
        traces_pix = [
            self.frame["elements"][tr_n].verticesPix.copy()
            for tr_n in self.frame["traces"]
        ]
        trace_let_pix = np.concatenate(traces_pix)

        self._create_trial_res(
            trace_let,
            trial_time,
            ptt,
            start_t_stamp,
    ...
    def _run_trial_main_loop(self, clock, time_bar_x, stim_idx: int):
        ...
        while True:
            ...
            if mouse.isPressedIn(self.frame["elements"]["start_point"]):
                ...
                tic = clock.getTime()
                self.draw_and_flip(exclude=["trace", "instructions", "the_box"])
                break
    ...
    def _adjust_time_bar(self, ratio, time_bar_x):
        ...
        new_size = [
            time_bar_x * ratio,  # change the x value
            self.frame["elements"]["time_bar"].size[1],
        ]
        new_pos = [
            (-time_bar_x * ratio / 2) + time_bar_x / 2,
    ...
    def _draw_trace(self, new_pos, new_trace=False):
        ...
        if new_trace:
            ...
            self.frame["elements"][tr_i_n] = create_element(
                "trace", win=self.win, vertices=np.zeros(shape=(1, 2))
            )
            self.frame["traces"].append(tr_i_n)

        # set the trace
        self.frame["elements"][self.frame["traces"][-1]].vertices = self.frame[
            "trace_vertices"
        ][self.frame["start_frame_idx"] : self.frame["idx"] + 1]
    ...
    def _exec_drawing(self, trial_timer, mouse, time_bar_x, cursor_t):
        ...
        while trial_timer.getTime() > 0:
            ...
            if not mouse.getPressed()[0] and self.block_settings["finish_when_raised"]:
    ...
    def _create_trial_res(
        ...
        self.trial_results["trial_duration_config"] = self.trial_settings[
            "trial_duration"
        ]
        self.trial_results["flip"] = self.stimuli["flip"]
        self.trial_results["theBox"] = self.frame["elements"]["the_box"].vertices.copy()

        self.trial_results["theBoxPix"] = self.frame["elements"]["the_box"].verticesPix
        ...
        self.trial_results["template_pix"] = convertToPix(
            self.stimuli["current_stim"], units="norm", pos=stim_pos, win=self.win
        )
    ...
    def get_fixation(self):
        return visual.Circle(
            self.win, radius=5, color="red", interpolate=True, units="pix"
        )

    def get_photobox(self):
        rect_white = visual.Rect(
            win=self.win, units="pix", width=40, height=40, fillColor=[1, 1, 1]
        )
